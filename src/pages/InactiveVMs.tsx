...
const InactiveVMs = () => {
  ...
  const loadInactiveVMs = async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to load inactive VMs";
      toast({
        title: "Error",
  ...
  const filterVMs = () => {
    const filtered = inactiveVMs.filter(
      (vm) =>
  ...
  const getPreCheckBadge = (preCheck: string) => {
    ...
    if (preCheckLower === "healthy")
      return (
        <Badge variant="default" className="bg-success text-white">
          Healthy
        </Badge>
      );
    if (preCheckLower === "actionrequired")
      return <Badge variant="destructive">Action Required</Badge>;
    if (preCheckLower === "passed")
      return (
        <Badge variant="default" className="bg-success text-white">
          Passed
        </Badge>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              VMs requiring backup attention
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-center">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">
              Inactive VMs Details
            </h1>
            {subscriptionParam && (
              <p className="text-muted-foreground mt-1">
                Filtered for subscription:{" "}
                <span className="font-medium">{subscriptionParam}</span>
              </p>
            )}
          </div>
        </div>
                  ...
                  <TableBody>
                    {filteredVMs.map((vm, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {getPreCheckBadge(vm.backupPreCheck)}
                        </TableCell>
                        <TableCell>
                          {getStatusBadge(vm.lastBackupStatus)}
                        </TableCell>
                        <TableCell>
                          {new Date(vm.latestRestorePoint).toLocaleDateString()}
                        </TableCell>
